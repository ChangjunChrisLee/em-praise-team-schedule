...
function mergeWithEmptyAssignments(assignments, sundays) {
  ...
  Object.keys(empty).forEach((key) => {
    if (assignments?.[key]) {
      ROLE_ORDER.forEach((role) => {
        empty[key][role] = assignments[key][role] || [];
      });
    }
  });
  return empty;
...
function countAssignments(assignments, name) {
  ...
  Object.values(assignments).forEach((day) => {
    Object.values(day).forEach((names) => {
...
function countAssignmentsByRole(assignments, name) {
  ...
  Object.values(assignments).forEach((day) => {
    ROLE_ORDER.forEach((role) => {
      if ((day[role] || []).includes(name)) counts[role] += 1;
    });
  });
...
function saveLocalData(data) {
  ...
  try {
    ...
    // localStorage 저장 실패 시 조용히 무시합니다.
  }
}
...
async function loadRemoteData() {
  return jsonpRequest({ action: "load" });
}

async function saveRemoteData(data) {
  return jsonpRequest({ action: "save", data: JSON.stringify(data) });
}

export default function PraiseTeamSelfScheduler() {
  ...
  const initialPeople = savedData?.people?.length
    ? savedData.people.sort((a, b) => a.localeCompare(b, "ko"))
    : INITIAL_PEOPLE;
  ...
  const appData = useMemo(
    () => ({ year, month, people, assignments: normalizedAssignments, selectedName }),
    [year, month, people, normalizedAssignments, selectedName]
  );
  ...
  const changeMonth = (nextMonth) => {
    ...
    setYear(y);
    setMonth(m);
    const newSundays = getSundays(y, m);
    setAssignments((prev) => ({ ...makeEmptyAssignments(newSundays), ...prev }));
  };
  ...
  const toggleAssignment = (key, role) => {
    setAssignments((prev) => {
      ...
      if (alreadyInThisRole) {
        day[role] = current.filter((n) => n !== selectedName);
        setMessage(`${selectedName}님의 ${role} 배정을 취소했습니다.`);
        return next;
      }

      if (assignedElsewhereThatDay) {
        setMessage(`${selectedName}님은 같은 주차에 이미 다른 포지션으로 배정되어 있습니다.`);
        return prev;
      }

      if (current.length >= ROLE_LIMITS[role]) {
        setMessage(`${role} 포지션 정원이 이미 찼습니다.`);
        return prev;
      }
  ...
  const resetAll = () => {
    setAssignments(makeEmptyAssignments(sundays));
    setMessage("이번 달 배정을 초기화했습니다.");
  };
  ...
  const addMember = () => {
    ...
    if (people.includes(name)) {
      ...
      setMessage(`${name}님은 이미 등록되어 있습니다.`);
      setNewMemberName("");
      return;
    }
    ...
    setSelectedName(name);
    setNewMemberName("");
    setMessage(`${name}님을 새 멤버로 추가했습니다.`);
  };

  const copyText = async () => {
    await navigator.clipboard.writeText(buildShareText(sundays, normalizedAssignments));
    setMessage("카카오톡 공유용 일정표를 복사했습니다.");
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 text-slate-900 md:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
            ...
            <CardContent className="p-4">
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setMessage(`${person}님을 선택했습니다.`);
                      }}
                      className={`rounded-full px-3 py-2 text-sm transition ${active ? "bg-slate-900 text-white shadow" : "bg-white text-slate-700 shadow-sm hover:bg-slate-100"}`}
                    >
                      {person}
                    </button>
                  );
              ...
              <div className="mt-4 flex flex-col gap-2 border-t border-slate-100 pt-4 sm:flex-row">
                <input
                  ...
                  placeholder="새 멤버 이름"
                  className="flex-1 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-slate-400"
                />
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            {lastSavedAt && (
              <div className="rounded-xl bg-white px-3 py-2 text-xs text-slate-500 shadow-sm">
                저장됨: {new Date(lastSavedAt).toLocaleString("ko-KR")}
              </div>
            )}
            <Button variant="outline" onClick={saveToSheet} disabled={isSaving || !hasUnsavedChanges}>
              <Save size={16} className="mr-1" /> {isSaving ? "저장 중" : "저장"}
            </Button>
                ...
                <CardContent className="p-4">
                  <div className="mb-4 flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-sm text-slate-500">주일예배</div>
                    </div>
                    <div className="rounded-full bg-slate-100 px-3 py-1 text-sm text-slate-600">
                      {Object.values(day).reduce((s, names) => s + names.length, 0)}명 배정
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    {ROLE_ORDER.map((role) => {
                      ...
                      return (
                        <div key={role} className="grid grid-cols-[130px_1fr_auto] items-center gap-2 rounded-xl bg-slate-50 p-2">
                          <div className="font-medium text-slate-700">{role}</div>
                          <div className="min-h-8">
                            {names.length ? (
                              <div className="flex flex-wrap gap-1">
                                {names.map((name) => (
                                  <span key={name} className={`rounded-full px-2 py-1 text-xs ${name === selectedName ? "bg-slate-900 text-white" : "bg-white text-slate-700 shadow-sm"}`}>{name}</span>
                                ))}
                              </div>
                            ) : (
                          ...
                          <Button
                            ...
                            disabled={disabled}
                            onClick={() => toggleAssignment(key, role)}
                          >
                            {selectedHere ? "취소" : "들어가기"}
                          </Button>
                        </div>
                      );
                    })}
        ...
        <Card className="rounded-2xl border-none shadow-sm">
          <CardContent className="p-4">
            <div className="mb-3 flex items-center gap-2 font-semibold"><AlertTriangle size={18} /> Serving Summary</div>
            <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-4">
              {people.map((person) => {
                ...
                return (
                  <div key={person} className="rounded-xl bg-slate-50 p-3">
                    <div className="flex items-center justify-between gap-2">
                      <div className="font-semibold">{person}</div>
                      <div className="rounded-full bg-white px-2 py-1 text-sm font-bold shadow-sm">총 {count}회</div>
                    </div>
                    <div className="mt-2 flex flex-wrap gap-1">
                      {activeRoles.length ? (
                        activeRoles.map((role) => (
                          <span key={role} className="rounded-full bg-white px-2 py-1 text-xs text-slate-700 shadow-sm">
                            {role} {roleCounts[role]}회
                          </span>
                        ))
                      ) : (
                        <span className="text-sm text-slate-400">아직 배정 없음</span>
                      )}
                    </div>
                  </div>
                );
